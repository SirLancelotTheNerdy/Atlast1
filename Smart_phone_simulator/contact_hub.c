...
int load_contacts(Contact contacts[], const char *filename) {
    ...
    while (fgets(line, sizeof(line), file) && i < MAX_CONTACTS) {
        ...
        if (token != NULL) strncpy(contacts[i].name, token, MAX_CONTACT_NAME_LENGTH);

        token = strtok(NULL, ",");
        if (token != NULL) strncpy(contacts[i].phone, token, MAX_PHONE_NUMBER_LENGTH);

        token = strtok(NULL, ",");
        if (token != NULL) strncpy(contacts[i].email, token, MAX_EMAIL_ADDRESS_LENGTH);
